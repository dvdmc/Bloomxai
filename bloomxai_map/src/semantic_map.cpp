...
VoxelGrid<SemanticMap::SemCellT>& SemanticMap::grid() { return _grid; }
...
const SemanticMap::Options& SemanticMap::options() const { return _options; }

void SemanticMap::setOptions(const Options& options) { _options = options; }

void SemanticMap::addHitPoint(const Vector3D& point,
                              const SemanticMap::VSemanticProb& semantics) {
  const auto coord = _grid.posToCoord(point);
  SemCellT* cell =
      SemanticMap::ensureCellInitalized(_accessor.value(coord, true));
...
void SemanticMap::addMissPoint(const Vector3D& point) {
  ...
  if (cell->update_id != _update_count) {
    cell->occ_prob_log = std::max(cell->occ_prob_log + _options.prob_miss_log,
                                  _options.clamp_min_log);

    // Make the semantics more uncertain
    cell->sem_prob_log = SemanticMap::vlogods(SemanticMap::regularizeSemantic(
        SemanticMap::vprob(cell->sem_prob_log)));
...
SemanticMap::VSemanticProb SemanticMap::regularizeSemantic(
  ...
  return (1 - _options.alpha_reg) * input +
         _options.alpha_reg * _options.prob_reg;
}
...
void SemanticMap::getOccupiedVoxelsAndClass(std::vector<CoordT>& coords, std::vector<int>& classes) {
  coords.clear();
  classes.clear();
  auto visitor = [&](SemCellT& cell, const CoordT& coord) {
    if (cell.occ_prob_log > _options.occupancy_threshold_log) {
